function [] = attitudeAnimation(A, P, theta, stlName, percent_start, step_size, animation_length, cam_choice)

% creates an animation with attitude and position
%
% [] = attitudeAnimation(A_BN, P, theta, stlName, percent_start, step_size, animation_length, cam_choice)
%
% Input arguments:
% ----------------------------------------------------------------
% A                     [3x3xN]     A_BN matrix                     [-]
% P                     [Nx3]       Position vector                 [-]
...
for i = n_start:step_size:n_end
    k = k + 1;
    [TR_frame1] = stlHandle_internal(TR, A(:,:,i)', P(i,:));
...
for i=n_start:step_size:n_end
    ...
    vect = 200*[A(1,:,i); 0,0,0] + [P(i,:); P(i,:)];
    b = plot3(vect(:,1), vect(:,2), vect(:,3), Color="red"); % x
    vect = 200*[A(2,:,i); 0,0,0] + [P(i,:); P(i,:)];
    c = plot3(vect(:,1), vect(:,2), vect(:,3), Color="green"); % y
    vect = 200*[A(3,:,i); 0,0,0] + [P(i,:); P(i,:)];
